---
title: 使用WildCard绑定OpenAI API以及OpenAI Key申请最新详细教程
description: 如何能调用 OpenAI API，OpenAI 如何绑卡？如何申请 Key，如何通过手机验证？
meta: 
  - 
    name: description
    content: 如何能调用 OpenAI API，OpenAI 如何绑卡？如何申请 Key，如何通过手机验证？
  - 
    name: keywords
    content: OpenAI API充值订阅
tags: 
  - OpenAI API充值订阅
...
# 使用WildCard绑定OpenAI API以及OpenAI Key申请最新详细教程
## 温馨提醒
在OpenAI绑卡前，一定要保证卡上至少有5美元的余额，否则会绑卡失败

## 登录 OpenAI platform

点击[platform.openai.com](platform.openai.com)，打开页面后然后输入你的OpenAI帐号和密码(也就是你的ChatGPT帐号)

![](https://hlplch.aliyuntm.com/chatgpt/WX20241110-193851%402x.png)

## 为OpenAI绑定付款方式

登录以后，点击右上角的  Settings，然后点击billing ，就可以进入充值入口。或者直接[访问地址](https://platform.openai.com/settings/organization/billing/overview)
...
点击菜单中的“API Keys”就可以看到已经创建的API Keys以及添加API Key的按钮，你也可以直接输入这个[链接创建API Keys](https://platform.openai.com/api-keys)
...
如果连续2次扣费失败（比如遇到卡内余额不足），OpenAI 就会封禁绑定的卡号。

API调用的消费情况[链接](https://platform.openai.com/usage)
![](https://hlplch.aliyuntm.com/chatgpt/WX20241110-194508%402x.png)
API调用额度限制，和支持的模型（GPT-3.5/GPT-4）[链接](https://platform.openai.com/settings/organization/limits)
